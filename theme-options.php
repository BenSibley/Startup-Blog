...
function ct_startup_blog_options_content() {
	...
	<div id="startup-blog-dashboard-wrap" class="wrap">
		<h2>
			<?php
			// translators: %s = theme name
			printf( esc_html__( '%s Dashboard', 'startup-blog' ), wp_get_theme( get_template() ) );
			?>
		</h2>
		...
		<div class="content-boxes">
			<div class="content content-support">
				...
				<p>
					<?php
					// translators: %s = theme name
					printf( esc_html__( 'Not sure where to start? The %1$s Support Center is filled with tutorials that will take you step-by-step through every feature in %1$s.', 'startup-blog' ), wp_get_theme( get_template() ) );
					?>
				</p>
				<p>
			...
			<?php if ( !function_exists( 'ct_startup_blog_pro_init' ) ) : ?>
				<div class="content content-premium-upgrade">
					...
					<p>
						<?php
						// translators: %s = theme name
						printf( esc_html__( 'Download the %s Pro plugin and unlock six new layouts, four post templates, advanced color controls, and more.', 'startup-blog' ), wp_get_theme( get_template() ) );
						?>
					</p>
					<p>
						<a target="_blank" class="button-primary"
			...
			<div class="content content-review">
				...
				<p>
					<?php
					// translators: %s = theme name
					printf( esc_html__( 'Help others find %s by leaving a review on wordpress.org.', 'startup-blog' ), wp_get_theme( get_template() ) );
					?>
				</p>
			...
			<div class="content content-delete-settings">
				...
				<p>
					...
					printf( __( '<strong>Warning:</strong> Clicking this button will erase the %2$s theme\'s current settings in the <a href="%1$s">Customizer</a>.', 'startup-blog' ), esc_url( $customizer_url ), wp_get_theme( get_template() ) ); 
					?>
				</p>
				<form method="post">
