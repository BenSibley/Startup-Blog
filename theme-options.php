...
function ct_business_blog_register_theme_page() {
	add_theme_page( sprintf( __( '%s Dashboard', 'business-blog' ), wp_get_theme( get_template() ) ), sprintf( __( '%s Dashboard', 'business-blog' ), wp_get_theme( get_template() ) ), 'edit_theme_options', 'business-blog-options', 'ct_business_blog_options_content', 'ct_business_blog_options_content' );
}
...
function ct_business_blog_options_content() {
	...
	<div id="business-blog-dashboard-wrap" class="wrap">
		<h2><?php printf( __( '%s Dashboard', 'business-blog' ), wp_get_theme( get_template() ) ); ?></h2>
		<?php do_action( 'ct_business_blog_theme_options_before' ); ?>
		<div class="content-boxes">
			<div class="content content-support">
				<h3><?php _e( 'Get Started', 'business-blog' ); ?></h3>
				<p><?php printf( __( 'Not sure where to start? The %1$s Support Center is filled with tutorials that will take you step-by-step through every feature in %1$s.', 'business-blog' ), wp_get_theme( get_template() ) ); ?></p>
				<p>
			...
			<?php if ( !function_exists( 'ct_business_blog_pro_init' ) ) : ?>
				<div class="content content-premium-upgrade">
					<h3><?php printf( __( 'Business Blog Pro', 'business-blog' ), wp_get_theme( get_template() ) ); ?></h3>
					<p><?php printf( __( 'Download the %s Pro plugin and unlock custom colors, new fonts, sliders, and more', 'business-blog' ), wp_get_theme( get_template() ) ); ?>...</p>
					<p>
						<a target="_blank" class="button-primary"
						   href="https://www.competethemes.com/business-blog-pro/"><?php _e( 'See Full Feature List', 'business-blog' ); ?></a>
					</p>
				</div>
			<?php endif; ?>
			<div class="content content-review">
				<h3><?php _e( 'Leave a Review', 'business-blog' ); ?></h3>
				<p><?php printf( __( 'Help others find %s by leaving a review on wordpress.org.', 'business-blog' ), wp_get_theme( get_template() ) ); ?></p>
			...
			<div class="content content-presspad">
				<h3><?php _e( 'Turn Business Blog into a Mobile App', 'business-blog' ); ?></h3>
				<p><?php printf( __( '%s can be converted into a mobile app and listed on the App Store with the help of PressPad News. Read our tutorial to learn more.', 'business-blog' ), wp_get_theme( get_template() ) ); ?></p>
